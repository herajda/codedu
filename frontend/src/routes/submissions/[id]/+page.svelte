...
{:else}
  <div class="space-y-6">
    <div class="card bg-base-100 shadow">
      <div class="card-body space-y-4">
        <div class="flex items-start justify-between gap-6">
          <div class="space-y-2">
            <h1 class="card-title text-2xl">{assignmentTitle || t('frontend/src/routes/submissions/[id]/+page.svelte::assignment_fallback_title')}</h1>
            <div class="flex flex-wrap gap-2 text-xs sm:text-sm opacity-80">
              <span class="inline-flex items-center gap-2 px-3 py-1 rounded bg-base-200">{t('frontend/src/routes/submissions/[id]/+page.svelte::attempt_prefix')}{submission.attempt_number ?? submission.id}</span>
              <span class="inline-flex items-center gap-2 px-3 py-1 rounded bg-base-200">{t('frontend/src/routes/submissions/[id]/+page.svelte::submitted_prefix')} {formatDateTime(submission.created_at)}</span>
              {#if assignmentManual}
                <span class="inline-flex items-center gap-2 px-3 py-1 rounded bg-info/20 text-info">{t('frontend/src/routes/submissions/[id]/+page.svelte::manual_review_badge')}</span>
              {/if}
            </div>
          </div>
          <div class="flex items-center gap-2">
            ...
            {#if submission.manually_accepted}
              <span class="badge badge-outline badge-success" title={t('frontend/src/routes/submissions/[id]/+page.svelte::accepted_by_teacher_title')}>{t('frontend/src/routes/submissions/[id]/+page.svelte::manually_accepted_badge')}</span>
            {/if}
            <button class="btn btn-ghost" on:click={goBack}>{t('frontend/src/routes/submissions/[id]/+page.svelte::back_button')}</button>
            <button class="btn btn-primary" on:click={openFiles}>{t('frontend/src/routes/submissions/[id]/+page.svelte::view_files_button')}</button>
          </div>
        </div>
        ...
        {#if (role==='teacher' || role==='admin')}
          <div class="rounded-box bg-base-200 p-4 mt-2">
            <div class="font-medium mb-2">{t('frontend/src/routes/submissions/[id]/+page.svelte::teacher_actions_title')}</div>
            ...
            {#if submission?.manually_accepted}
              ...
              <div class="flex items-center gap-3">
                ...
                }}>{t('frontend/src/routes/submissions/[id]/+page.svelte::undo_manual_acceptance_button')}</button>
              </div>
              <div class="text-xs opacity-70 mt-2">{t('frontend/src/routes/submissions/[id]/+page.svelte::undo_manual_acceptance_description')}</div>
            {:else}
              ...
              {#if submission?.status === 'failed' || submission?.status === 'pending'}
                ...
                <div class="collapse collapse-arrow bg-base-100">
                  ...
                  <div class="collapse-title font-medium">
                    {t('frontend/src/routes/submissions/[id]/+page.svelte::accept_and_give_points_title')}
                  </div>
                  <div class="collapse-content">
                    <div class="flex items-center gap-3 mt-3">
                      <input type="number" step="1" min="0" inputmode="numeric" pattern="[0-9]*" on:keydown={(e) => { if (['e','E','+','-','.',','].includes(e.key)) e.preventDefault() }} class="input input-bordered input-sm w-28 sm:w-32" bind:value={overrideValue} placeholder={t('frontend/src/routes/submissions/[id]/+page.svelte::points_optional_placeholder')} aria-label={t('frontend/src/routes/submissions/[id]/+page.svelte::override_points_aria_label')}>
                      <button class={`btn btn-primary btn-sm ${savingOverride ? 'loading' : ''}`} on:click={saveOverride} disabled={savingOverride}>{t('frontend/src/routes/submissions/[id]/+page.svelte::save_points_button')}</button>
                      <button class="btn btn-success btn-sm" on:click={async()=>{ try{ const raw:any=overrideValue; const str = raw==null? '' : (typeof raw==='string'? raw : String(raw)); const v = str.trim()===''? null : parseInt(str,10); await apiFetch(`/api/submissions/${submission.id}/accept`, { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ points: v })}); await load(); }catch(e:any){ err=e.message } }}>{t('frontend/src/routes/submissions/[id]/+page.svelte::accept_submission_button')}</button>
                    </div>
                    <div class="text-xs opacity-70 mt-2">{t('frontend/src/routes/submissions/[id]/+page.svelte::accept_submission_description')}</div>
                  </div>
                </div>
              {:else}
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div class="stat bg-base-200 rounded-box">
            <div class="stat-title">{t('frontend/src/routes/submissions/[id]/+page.svelte::stat_tests')}</div>
            <div class="stat-value text-base">{totalTests}</div>
          </div>
          <div class="stat bg-base-200 rounded-box">
            <div class="stat-title">{t('frontend/src/routes/submissions/[id]/+page.svelte::stat_passed')}</div>
            <div class="stat-value text-success">{passedCount}</div>
          </div>
          <div class="stat bg-base-200 rounded-box">
            <div class="stat-title">{t('frontend/src/routes/submissions/[id]/+page.svelte::stat_limited')}</div>
            <div class="stat-value text-warning">{warnedCount}</div>
          </div>
          <div class="stat bg-base-200 rounded-box">
            <div class="stat-title">{t('frontend/src/routes/submissions/[id]/+page.svelte::stat_failed')}</div>
    ...
    {#if showAutoUI}
      <div class="card bg-base-100 shadow">
        <div class="card-body">
          <h3 class="card-title">{t('frontend/src/routes/submissions/[id]/+page.svelte::results_title')}</h3>
                      ...
                      <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex flex-wrap items-center gap-2 text-sm sm:text-base font-medium">
                          ...
                          {#if allowTestDetails && mode === 'function'}
                            <span class="badge badge-outline badge-info text-xs font-semibold tracking-wide uppercase">{t('frontend/src/routes/submissions/[id]/+page.svelte::function_badge')}</span>
                            {#if r.function_name}
                          ...
                          {:else if allowTestDetails && (typeof r.stdin !== 'undefined' || typeof r.expected_stdout !== 'undefined')}
                            <span class="badge badge-outline text-xs font-semibold tracking-wide uppercase">{t('frontend/src/routes/submissions/[id]/+page.svelte::io_test_badge')}</span>
                          {/if}
                        </div>
                        <div class="flex items-center flex-wrap gap-3 text-xs sm:text-sm">
                          ...
                          <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-base-300">
                            ...
                            {r.runtime_ms} {t('frontend/src/routes/submissions/[id]/+page.svelte::milliseconds_unit')}
                          </span>
                          <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-base-300">
                            <svg class="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                            ...
                            {t('frontend/src/routes/submissions/[id]/+page.svelte::exit_code_prefix')}{r.exit_code}
                          </span>
                        </div>
                      </div>
                    </summary>
                      ...
                      {#if allowTestDetails}
                        <section class="rounded-2xl border border-base-300/70 bg-base-100 p-4 shadow-sm">
                          ...
                          {#if mode === 'function'}
                            <div class="grid gap-3 md:grid-cols-2">
                              <div class="rounded-xl border border-base-300/60 bg-base-200/70 p-3">
                                <div class="text-xs font-semibold uppercase tracking-wide text-base-content/70">{t('frontend/src/routes/submissions/[id]/+page.svelte::function_label')}</div>
                                <div class="mt-2 font-mono text-sm">{r.function_name}</div>
                              </div>
                              <div class="rounded-xl border border-base-300/60 bg-base-200/70 p-3">
                                <div class="text-xs font-semibold uppercase tracking-wide text-base-content/70">{t('frontend/src/routes/submissions/[id]/+page.svelte::expected_return_label')}</div>
                                <pre class="mt-2 whitespace-pre-wrap font-mono text-sm leading-relaxed">{r.expected_return ?? t('frontend/src/routes/submissions/[id]/+page.svelte::empty_symbol')}</pre>
                              </div>
                              <div class="rounded-xl border border-base-300/60 bg-base-200/70 p-3">
                                <div class="text-xs font-semibold uppercase tracking-wide text-base-content/70">{t('frontend/src/routes/submissions/[id]/+page.svelte::arguments_label')}</div>
                                <pre class="mt-2 whitespace-pre-wrap font-mono text-sm leading-relaxed">{r.function_args ?? t('frontend/src/routes/submissions/[id]/+page.svelte::empty_array')}</pre>
                              </div>
                              <div class="rounded-xl border border-base-300/60 bg-base-200/70 p-3">
                                <div class="text-xs font-semibold uppercase tracking-wide text-base-content/70">{t('frontend/src/routes/submissions/[id]/+page.svelte::keyword_args_label')}</div>
                                <pre class="mt-2 whitespace-pre-wrap font-mono text-sm leading-relaxed">{r.function_kwargs ?? t('frontend/src/routes/submissions/[id]/+page.svelte::empty_object')}</pre>
                              </div>
                              {#if r.actual_return}
                                <div class="rounded-xl border border-base-300/60 bg-base-200/70 p-3 md:col-span-2">
                                  <div class="text-xs font-semibold uppercase tracking-wide text-base-content/70">{t('frontend/src/routes/submissions/[id]/+page.svelte::actual_return_label')}</div>
                          ...
                          {:else if typeof r.stdin !== 'undefined' || typeof r.expected_stdout !== 'undefined'}
                            <div class="grid gap-3 md:grid-cols-2">
                              <div class="rounded-xl border border-base-300/60 bg-base-200/70 p-3">
                                <div class="text-xs font-semibold uppercase tracking-wide text-base-content/70">{t('frontend/src/routes/submissions/[id]/+page.svelte::input_label')}</div>
                                {#if typeof r.stdin !== 'undefined'}
                                  {#if r.stdin?.length}
                                    <pre class="mt-2 whitespace-pre-wrap font-mono text-sm leading-relaxed">{r.stdin}</pre>
                                  {:else}
                                    <div class="mt-2 text-sm italic opacity-60">{t('frontend/src/routes/submissions/[id]/+page.svelte::empty_input_message')}</div>
                                  {/if}
                                {:else}
                                  <div class="mt-2 text-sm italic opacity-60">{t('frontend/src/routes/submissions/[id]/+page.svelte::input_not_provided_message')}</div>
                                {/if}
                              </div>
                              <div class="rounded-xl border border-base-300/60 bg-base-200/70 p-3">
                                <div class="text-xs font-semibold uppercase tracking-wide text-base-content/70">{t('frontend/src/routes/submissions/[id]/+page.svelte::expected_output_label')}</div>
                                {#if typeof r.expected_stdout !== 'undefined'}
                                  ...
                                  {:else}
                                    <div class="mt-2 text-sm italic opacity-60">{t('frontend/src/routes/submissions/[id]/+page.svelte::empty_output_message')}</div>
                                  {/if}
                                {:else}
                                  <div class="mt-2 text-sm italic opacity-60">{t('frontend/src/routes/submissions/[id]/+page.svelte::output_not_provided_message')}</div>
                                {/if}
                              </div>
                            </div>
                          {:else}
                            <div class="text-sm opacity-70">{t('frontend/src/routes/submissions/[id]/+page.svelte::no_test_metadata_message')}</div>
                          {/if}
                        </section>
                      {/if}
                      {#if allowTraceback}
                        <section class="rounded-2xl border border-base-300/70 bg-base-100 p-4 shadow-sm">
                          ...
                          {:else}
                            <div class="text-sm italic opacity-60">{t('frontend/src/routes/submissions/[id]/+page.svelte::no_stderr_output_message')}</div>
                          {/if}
                        </section>
                      {/if}
                    </div>
          ...
          {:else}
            <div class="text-sm opacity-70 italic">{t('frontend/src/routes/submissions/[id]/+page.svelte::no_results_yet_message')}</div>
          {/if}
        </div>
      </div>
    {/if}
    ...
    {#if showLLM}
      <div class="card bg-base-100 shadow">
        <div class="card-body space-y-4">
          <h3 class="card-title">{t('frontend/src/routes/submissions/[id]/+page.svelte::llm_interactive_title')}</h3>
          {#if llm}
            <div class="grid md:grid-cols-3 gap-3">
              <div class="rounded-box bg-base-200 p-3">
                <div class="font-medium">{t('frontend/src/routes/submissions/[id]/+page.svelte::smoke_test_label')}</div>
                <div class="text-sm">{llm.smoke_ok ? t('frontend/src/routes/submissions/[id]/+page.svelte::smoke_test_ok') : t('frontend/src/routes/submissions/[id]/+page.svelte::smoke_test_failed')}</div>
              </div>
              <div class="rounded-box bg-base-200 p-3">
                <div class="font-medium">{t('frontend/src/routes/submissions/[id]/+page.svelte::verdict_label')}</div>
                <div class="text-sm">{llm.verdict ?? t('frontend/src/routes/submissions/[id]/+page.svelte::dash_placeholder')}</div>
              </div>
              <div class="rounded-box bg-base-200 p-3">
                <div class="font-medium">{t('frontend/src/routes/submissions/[id]/+page.svelte::reason_label')}</div>
                <div class="text-sm break-words">{llm.reason ?? t('frontend/src/routes/submissions/[id]/+page.svelte::dash_placeholder')}</div>
              </div>
            </div>

            {#if review && allowLLMDetails}
              <div class="rounded-box bg-base-200 p-4 space-y-3">
                <div class="font-semibold flex items-center gap-2">{t('frontend/src/routes/submissions/[id]/+page.svelte::llm_review_title')}</div>
                {#if review.summary}
                ...
                {#if Array.isArray(review.issues) && review.issues.length}
                  <div class="space-y-2">
                    <div class="font-medium">{t('frontend/src/routes/submissions/[id]/+page.svelte::issues_title')}</div>
                    <div class="space-y-2">
                      {#each review.issues as issue}
                        <div class="rounded bg-base-300 p-3 space-y-1">
                          ...
                          {#if issue.reproduction}
                            <div class="text-sm">
                              <div class="opacity-70">{t('frontend/src/routes/submissions/[id]/+page.svelte::reproduction_label')}</div>
                              ...
                              {#if issue.reproduction.expect_regex}
                                <div class="mt-1">{t('frontend/src/routes/submissions/[id]/+page.svelte::expect_label')}: <span class="font-mono">/{issue.reproduction.expect_regex}/</span></div>
                              {/if}
                ...
                {#if Array.isArray(review.suggestions) && review.suggestions.length}
                  <div class="space-y-1">
                    <div class="font-medium">{t('frontend/src/routes/submissions/[id]/+page.svelte::suggestions_title')}</div>
                    <ul class="list-disc list-inside">
                ...
                {#if review.acceptance}
                  <div class="pt-1">
                    <div class="font-medium">{t('frontend/src/routes/submissions/[id]/+page.svelte::acceptance_title')}</div>
                    <div class="flex items-center gap-2 text-sm">
                      <span class={`badge ${review.acceptance.ok ? 'badge-success' : 'badge-error'}`}>{review.acceptance.ok ? t('frontend/src/routes/submissions/[id]/+page.svelte::accepted_status') : t('frontend/src/routes/submissions/[id]/+page.svelte::rejected_status')}</span>
            ...
            {#if transcriptMsgs.length && allowLLMDetails}
              <div class="rounded-box bg-base-200 p-4 space-y-2">
                <div class="font-semibold">{t('frontend/src/routes/submissions/[id]/+page.svelte::interactive_transcript_title')}</div>
                <div class="space-y-2">
            ...
            {/if}
          {:else}
            <div class="text-sm opacity-70">{t('frontend/src/routes/submissions/[id]/+page.svelte::no_llm_data_yet_message')}</div>
          {/if}
        </div>
      </div>
    {/if}
      ...
      <div class="card bg-base-100 shadow">
        <div class="card-body space-y-3">
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h3 class="card-title">{t('frontend/src/routes/submissions/[id]/+page.svelte::manual_testing_title')}</h3>
              <p class="text-sm opacity-70">{t('frontend/src/routes/submissions/[id]/+page.svelte::manual_testing_description')}</p>
            </div>
          ...
          {:else}
            <div class="text-sm opacity-70 italic">{t('frontend/src/routes/submissions/[id]/+page.svelte::open_manual_console_message')}</div>
          {/if}
        </div>
      </div>
    {/if}
...
<dialog bind:this={fileDialog} class="modal">
  <div class="modal-box w-11/12 max-w-5xl">
    <div class="flex items-center justify-between mb-2">
      <div class="font-medium">{t('frontend/src/routes/submissions/[id]/+page.svelte::files_dialog_title')}</div>
      <button class="btn btn-sm btn-primary" on:click={downloadFiles}>{t('frontend/src/routes/submissions/[id]/+page.svelte::download_button')}</button>
    </div>
    {#if files.length}
  ...
  <form method="dialog" class="modal-backdrop"><button>{t('frontend/src/routes/submissions/[id]/+page.svelte::close_button')}</button></form>
</dialog>
