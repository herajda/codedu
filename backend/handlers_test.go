...
func TestGetAssignmentStudentForbidden(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "public_id", "title", "description", "created_by", "deadline", "max_points", "grading_policy", "published", "template_path", "created_at", "updated_at", "class_id"}).
		AddRow(4, "uid4", "A", "d", 1, now, 100, "all_or_nothing", true, nil, now, now, 2)
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT id, public_id, title, description, created_by, deadline, max_points, grading_policy, published, template_path, created_at, updated_at, class_id FROM assignments WHERE public_id = $1`)).
		WithArgs("uid4").WillReturnRows(rows)
	...
	c.Params = gin.Params{{Key: "id", Value: "uid4"}}
	c.Request, _ = http.NewRequest("GET", "/assignments/uid4", nil)
	c.Set("role", "student")
	c.Set("userID", 9)
...
func TestUpdateAssignmentTeacherForbidden(t *testing.T) {
	...
	row := sqlmock.NewRows([]string{"id", "public_id"}).AddRow(5, "uid5")
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT id, public_id, title, description, created_by, deadline, max_points, grading_policy, published, template_path, created_at, updated_at, class_id FROM assignments WHERE public_id = $1`)).
		WithArgs("uid5").WillReturnRows(row)
	...
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Params = gin.Params{{Key: "id", Value: "uid5"}}
	body := `{"title":"t","description":"","deadline":"2023-01-01T00:00:00Z","max_points":10,"grading_policy":"all_or_nothing"}`
	c.Request, _ = http.NewRequest("PUT", "/assignments/uid5", bytes.NewBufferString(body))
