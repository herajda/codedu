<script lang="ts">
  ...
  $: if ($page.params.id !== classId || $page.params.sid !== studentId) {
    classId = $page.params.id; studentId = $page.params.sid; load();
  }
  ...
  function completedCount() {
    return (cls?.assignments ?? []).filter((a: any) => bestFor(a.id) >= a.max_points).length;
  }

  function pointsEarned() {
    return (cls?.assignments ?? []).reduce((sum: number, a: any) => sum + bestFor(a.id), 0);
  }

  function pointsTotal() {
    return (cls?.assignments ?? []).reduce((sum: number, a: any) => sum + (a.max_points ?? 0), 0);
  }
  ...
  async function load() {
    ...
    try {
      ...
      const details = await Promise.all(assignments.map(a => apiJSON(`/api/assignments/${a.id}`).catch(() => null)));
      const rec: any[] = [];
      ...
      recent = rec.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 8);
        ...
        <ul class="space-y-2">
          {#each recent as r}
            <li class="flex items-center justify-between text-sm">
              <a href={`/assignments/${r.assignment_id}`} class="truncate">{r.title}</a>
              <span class="opacity-70 whitespace-nowrap">{formatDateTime(r.created_at)}</span>
            </li>
          {/each}
          {#if !recent.length}
